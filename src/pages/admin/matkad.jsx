...
const MatkaDetail = ({ valitudMatk, setValitudMatk }) => {
    return (
        <div style={{ display: 'flex', flexDirection: 'column', flexShrink: 1 }}>
            ...
            <input placeholder="Asukoha kirjeldus" value={valitudMatk.locationDsecription || ''} onChange={e => setValitudMatk(vanaMatk => ({ ...vanaMatk, locationDsecription: e.target.value }))} />
            ...
            <button style={{backgroundColor: 'black', borderRadius: 12, color: 'white', marginTop: 5}} onClick={() => {  }}>Salvesta</button>
        </div>
    )
}
...
const MatkadAdmin = () => {
    ...
    const [uusMatk, setUusMatk] = useState(null)
